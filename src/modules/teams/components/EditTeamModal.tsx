...
export function EditTeamModal({ isOpen, onClose, organizationId, onSuccess, team }: EditTeamModalProps) {
  ...
  useEffect(() => {
    if (isOpen && team) {
      setName(team.name || "");
      setCategory(team.category || team.typeCategory || "");
      setManagerId(team.manager?.id || team.manager?._id || team.manager || "");
      setIconIndex(team.iconIndex ?? 0);
      setErrors({});
    }
  }, [isOpen, team]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      const res = await TeamsService.updateTeam(team.id || team._id, {
        name: name.trim(),
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 sm:px-7 pt-5 pb-3">
          <Text as="h3" weight="bold" size="lg" className="tracking-tight" style={{ color: COLORS.text }}>Edit Team</Text>
          <button
            onClick={onClose}
          ...
          >
            {loading ? (
               <div className="flex items-center gap-2">
                 ...
                 <Text weight="bold" size="sm">Updating...</Text>
               </div>
             ) : (
               <Text weight="bold" size="sm">Update Team</Text>
             )}
          </button>
        </div>
      </div>
