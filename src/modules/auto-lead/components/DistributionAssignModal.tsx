...
export const DistributionAssignModal: React.FC<Props> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl flex flex-col max-h-[92vh] sm:max-h-[90vh] shadow-2xl overflow-hidden">
        <div className="flex items-center gap-3 px-4 sm:px-5 pt-4 sm:pt-5 pb-4 border-b border-gray-100">
          <div className="w-2 h-8 rounded-full" style={{ backgroundColor: COLORS.primary }} />
          <div className="flex-1 min-w-0">
            <Text weight="bold" className="text-black block truncate" style={{ fontSize: '16px' }}>
              {title}
            </Text>
        ...
        <div className="px-4 sm:px-5 py-3 border-b border-gray-100">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <button type="button" className="flex items-center gap-2 cursor-pointer text-left" onClick={handleSelectAll}>
              <div
                className={`w-5 h-5 rounded-[4px] flex items-center justify-center transition-colors ${
                  allSelected ? 'bg-[#1C3A76]' : 'border-2 border-gray-300'
                }`}
              >
            ...
            sorted.map(camp => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 sm:p-3.5 rounded-2xl border-[1.5px] cursor-pointer transition-all ${
                    isSelected ? 'bg-[#EEF3FC] border-[#1C3A76]/40 shadow-sm' : 'bg-white border-gray-100 hover:border-gray-200'
                  }`}
                >
                  <div
                    className={`w-9 h-9 flex-shrink-0 rounded-lg flex items-center justify-center text-[11px] font-bold ${
                  ...
                  <div className="flex-1 min-w-0">
                    <Text weight="semibold" className="text-gray-800 block truncate" style={{ fontSize: '14px' }}>
                      {camp.title}
                    </Text>
                    <Text className="text-gray-400 block" style={{ fontSize: '11px' }}>
                      {camp.leadsCount} leads
                    </Text>
                  </div>
                  <div
                    className={`w-6 h-6 rounded-[5px] flex-shrink-0 flex items-center justify-center transition-colors ${
        ...
        <div className="px-4 sm:px-5 pb-4 sm:pb-5 pt-3 border-t border-gray-100">
          ...
          >
            {applying ? (
              ...
              `Assign ${selectedIds.length} Ad${selectedIds.length !== 1 ? 's' : ''}`
            )}
          </button>
        </div>
      </div>
