...
export const CampaignSelectionArea: React.FC<Props> = ({
  ...
  // Redundant scroll trigger helper (if not compact)
  ...
  // Handle Select All for the CURRENT filtered campaigns
  ...
  const handleSelectAllToggle = () => {
    if (isAllFilteredSelected) {
      // Remove all currently filtered campaign IDs from selection
      ...
      // Add all currently filtered campaign IDs to selection
  ...
  return (
    <div className={compact ? 'p-0 space-y-4' : 'bg-white rounded-3xl p-6 shadow-sm mt-5 mx-6 flex-1 flex flex-col overflow-hidden border border-gray-100/30'}>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-gray-50/70 border border-gray-100 rounded-2xl p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-gray-400 font-medium uppercase tracking-wider">Total Active</p>
            <p className="text-[13px] font-bold text-gray-800">{totalCount} Campaigns</p>
          </div>
        </div>
        ...
        <div className="bg-[#1C3A76]/5 border border-[#1C3A76]/10 rounded-2xl p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-[#1C3A76]/75 font-medium uppercase tracking-wider">Selected</p>
            <p className="text-[13px] font-bold text-[#1C3A76]">{selectedCount} Ads Included</p>
          </div>
        </div>
        ...
        <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-emerald-600/75 font-medium uppercase tracking-wider">Lead Coverage</p>
            <p className="text-[13px] font-bold text-emerald-700">{totalLeadsCoverage.toLocaleString()} Leads</p>
          </div>
        </div>
      </div>

      {/* ─── Sleek Control Bar ─── */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50/40 border border-gray-100 rounded-2xl p-3">

        {/* Left Side: Select All & Tabs */}
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <label className="flex items-center gap-2 cursor-pointer shrink-0">
            ...
            />
            <div className={`w-5 h-5 rounded-[6px] flex-shrink-0 flex items-center justify-center transition-all ${isAllFilteredSelected
                ? 'bg-[#1C3A76]'
          ...
          {/* Filter Tabs */}
          <div className="flex bg-gray-100/80 p-0.5 rounded-xl border border-gray-200/40">
            <button
              onClick={() => setActiveFilter('all')}
              className={`px-3 py-1 rounded-lg text-[11.5px] font-bold transition-all cursor-pointer flex items-center gap-1.5 ${activeFilter === 'all'
                  ? 'bg-white text-gray-800 shadow-xs'
                  : 'text-gray-500 hover:text-gray-800'
                }`}
            >
              All
              <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-semibold ${activeFilter === 'all' ? 'bg-gray-100 text-gray-700' : 'bg-gray-200/60 text-gray-500'
                }`}>
                {totalCount}
              </span>
            </button>
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg text-[11.5px] font-bold transition-all cursor-pointer flex items-center gap-1.5 ${activeFilter === 'selected'
                  ? 'bg-white text-[#1C3A76] shadow-xs'
                  : 'text-gray-500 hover:text-gray-800'
                }`}
            >
              Selected
              <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-semibold ${activeFilter === 'selected' ? 'bg-[#1C3A76]/10 text-[#1C3A76]' : 'bg-gray-200/60 text-gray-500'
                }`}>
                {selectedCount}
              </span>
            </button>
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg text-[11.5px] font-bold transition-all cursor-pointer flex items-center gap-1.5 ${activeFilter === 'unselected'
                  ? 'bg-white text-gray-800 shadow-xs'
                  : 'text-gray-500 hover:text-gray-800'
                }`}
            >
              Unselected
              <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-semibold ${activeFilter === 'unselected' ? 'bg-gray-200 text-gray-700' : 'bg-gray-200/60 text-gray-500'
                }`}>
                {unselectedCount}
              </span>
            </button>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-[200px]">
            <input
              ...
              className="w-full bg-white border border-gray-200 rounded-xl py-1.5 pl-8 pr-4 text-[12.5px] focus:outline-none focus:border-[#1C3A76] text-black shadow-xs focus:ring-1 focus:ring-[#1C3A76]"
            />
          ...
          <button
            ...
            className={`text-[12px] px-4 py-1.5 rounded-xl font-bold tracking-wide transition-all border shrink-0 ${selectedCount > 0
      ...
      <div
        ...
        className={`grid grid-cols-1 xl:grid-cols-2 gap-3 content-start custom-scrollbar ${compact ? 'max-h-[300px] overflow-y-auto pr-1' : 'flex-1 overflow-y-auto pr-2 pb-4'
          }`}
        style={{
          scrollbarWidth: 'thin',
      ...
      {/* Dynamic Scroll Indicator for premium look */}
